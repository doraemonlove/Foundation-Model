...
class Exp_Forecast(Exp_Basic):
        ...
        for epoch in range(self.args.finetune_epochs):
            ...
            for i, (batch_x, batch_y, batch_x_mark, batch_y_mark) in enumerate(finetune_loader):
                ...
                if i % 50 == 0:
                    ...
                    print(
                        "\titers: {0}, epoch: {1} | loss: {2:.7f} | cost_time: {3:.0f} | memory: allocated {4:.0f}MB, reserved {5:.0f}MB, cached {6:.0f}MB | cpu_percent: {7:.2f}%"
    ...
    def test(self, setting, test=0):
        ...
        if self.args.use_finetune_model:
        ...
        with torch.no_grad():
            for output_ptr in range(len(self.args.output_len_list)):
                ...
                for i, (batch_x, batch_y, batch_x_mark, batch_y_mark) in enumerate(test_loader):
                    ...
                    if i % 50 == 0:  # 每10个batch打印一次
                        cost_time = time.time() - time_now
                        cpu_percent = psutil.cpu_percent()
                        print(
                            "\ttest iters: {0} | cost_time: {1:.0f}s | memory: allocated {2:.0f}MB, reserved {3:.0f}MB, cached {4:.0f}MB | cpu_percent: {5:.2f}%"
                            .format(i, cost_time,
                                    torch.cuda.memory_allocated() / 1024 / 1024,
                                    torch.cuda.memory_reserved() / 1024 / 1024,
                                    torch.cuda.memory_cached() / 1024 / 1024,
        ...
        if self.args.output_len_list is not None:
            for i in range(len(preds_list)):
                preds = preds_list[i]
                trues = trues_list[i]
                output_len = self.args.output_len_list[i]
                preds = torch.cat(preds, dim=0).numpy()
                trues = torch.cat(trues, dim=0).numpy()
                mae, mse, smape, mape, mase = metric(preds, trues)
                

                f = open("result_forecast.txt", 'a')
                f.write(f"{setting} output_len:{output_len}\n")
                f.write('mse:{:.6f}, mae:{:.6f}, smape:{:.6f}, mape:{:.6f}, mase:{:.6f}'.format(mse, mae, smape, mape, mase))
                f.write('\n')
                f.write('\n')
                f.close()
