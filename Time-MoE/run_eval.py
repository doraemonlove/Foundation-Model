...
class MAPEMetric(SumEvalMetric):
    def _calculate(self, preds, labels, **kwargs):
        epsilon = 1e-8  # 防止除以零
        return torch.sum(torch.abs((preds - labels) / (labels + epsilon))) * 100


class SMAPEMetric(SumEvalMetric):
    def _calculate(self, preds, labels, **kwargs):
        epsilon = 1e-8  # 防止除以零
        numerator = torch.abs(preds - labels)
        denominator = (torch.abs(preds) + torch.abs(labels)) / 2 + epsilon
        return torch.sum(numerator / denominator) * 100


class MASEMetric(SumEvalMetric):
    def __init__(self, insample_data, *args, **kwargs):
        super().__init__(*args, **kwargs)
        self.insample_data = insample_data  # 训练集数据，用于计算MASE的分母

    def _calculate(self, preds, labels, **kwargs):
        epsilon = 1e-8  # 防止除以零
        # 计算 naive 的误差（通常是后移一位的差异绝对值）
        naive_diff = torch.sum(torch.abs(self.insample_data[1:] - self.insample_data[:-1])) / (len(self.insample_data) - 1)
        return torch.sum(torch.abs(preds - labels)) / (naive_diff + epsilon)
...
def evaluate(args):
    ...
    with torch.no_grad():
        for batch in train_dl:
            insample_data.append(batch['inputs'])  # 保留为 tensor，而不是转为 numpy
    insample_data = torch.cat(insample_data, dim=0).to(device)  

    # evaluation
    metric_list = [
        MSEMetric(name='mse'),
        MAEMetric(name='mae'),
        MAPEMetric(name='mape'),
        SMAPEMetric(name='smape'),
        MASEMetric(insample_data=insample_data, name='mase')
    ]
