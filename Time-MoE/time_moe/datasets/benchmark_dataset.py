...
class BenchmarkEvalDataset(Dataset):
    ...
    def __init__(self, csv_path, context_length: int, prediction_length: int, timestamp: str = 'date', target: str = 'OT'):
        super().__init__()
        ...
        if 'etth' in base_name:
            ...
            num_train = int(len(df) * 0.1)
            num_test = int(len(df) * 0.85)
            num_vali = len(df) - num_train - num_test
            border1s = [0, num_train - context_length, len(df) - num_test - context_length]
            border2s = [num_train, num_train + num_vali, len(df)]

        start_dt = df.iloc[border1s[2]][self.timestamp]
        eval_start_dt = df.iloc[border1s[2] + context_length][self.timestamp]
        end_dt = df.iloc[border2s[2] - 1][self.timestamp]
        print(f'>>> Split test data from {start_dt} to {end_dt}, '
              f'and evaluation start date is: {eval_start_dt}')

        # cols = df.columns[1:]
        # df_values = df[cols].values
        df_values = df[[self.target]].values
