...
class ChronosBoltModelForForecasting(T5PreTrainedModel):
    ...
    def forward(
        ...
        # Unscale predictions
        # 修改：原代码会出现quantile_preds_shape未定义错误，这里将其修正
        ...
        # Unscale predictions
        quantile_preds = self.instance_norm.inverse(
            quantile_preds.view(batch_size, -1),  # 将 quantile_preds 展平为 (batch_size, num_quantiles * prediction_length)
            loc_scale,
        )

        # 定义 quantile_preds_shape
        quantile_preds_shape = (batch_size, self.num_quantiles, self.chronos_config.prediction_length)

        # 使用定义的形状
        quantile_preds = quantile_preds.view(*quantile_preds_shape)  # 将 quantile_preds 重新调整为 (batch_size, num_quantiles, prediction_length)
...
class ChronosBoltPipeline(BaseChronosPipeline):
    ...
    def predict(  # type: ignore[override]
        ...
        while remaining > 0:
            with torch.no_grad():
                prediction = self.model(context=context_tensor).quantile_preds
